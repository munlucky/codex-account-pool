...
Validation is turn-aware. Tool calls in the current user turn are strict because Gemini requires the current function-calling step to preserve its reasoning signature. Missing replay data from a completed older turn does not by itself abort a fresh user turn. The router-issued call handle is also the continuity authority for the provider wire-model variant: if a client changes only the reasoning-effort variant inside the same model family while signed tool history is active, the router keeps the original wire model for that conversation instead of invalidating the call or replaying its signature under a different variant. A change to a different model family still fails continuity validation. For parallel Gemini calls, the router restores the original model step, keeps the function calls together, keeps their results together, and places the thought signature only on the leading call for that step. Sequential model steps retain separate signatures.
...
The persistence file is `<state-root>/antigravity/continuity.json`. Writes use a temporary file, `fsync`, and atomic replacement; Unix mode is `0600`. It contains continuity metadata and provider thought signatures, but no OAuth access/refresh tokens, Cloud Code Assist project binding, raw prompt text or tool output. Tool arguments normally have only a digest; `exit_plan_mode` additionally stores original arguments (including plan text) so client-side approved-plan redaction can be replayed with the original signature. These originals use the same call retention and eviction policy. Legacy digest-only records remain readable but cannot restore redacted arguments.
