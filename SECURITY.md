...
Antigravity account policy stores profile IDs and bounded in-memory lease/quota observations, not OAuth tokens or Cloud Code Assist project bindings. Credentials are reloaded for the selected registered Google profile each request. `auth use` affects new conversations. Local `X-AI-Account` selectors require the existing router key, are stripped before backend dispatch, and never silently fall back.

Random tool-call handles bind profile, wire model, provider session, function name, argument digest, and Gemini step position. The corresponding provider wire ID and thought signature are persisted in `<state-root>/antigravity/continuity.json` so headerless clients can survive a normal router/container restart without exposing those values in `call_id`. The file is written atomically and uses Unix mode `0600`; treat the managed state root as sensitive local state. It does not contain OAuth access/refresh tokens, Cloud Code Assist project bindings, raw prompts, raw tool arguments, or tool results. Existing single-user/loopback requirements still apply. Tool history never triggers cross-account failover; invalid current-turn continuity fails locally.
